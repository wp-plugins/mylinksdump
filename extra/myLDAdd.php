 ...
 if (!empty($_POST['url']) && is_valid_url($_POST['url']) && !empty($_POST['title'])){
  ...
  if (!empty($current_user->user_login) && !empty($current_user->ID)){
     $sql_query = $wpdb->prepare("INSERT INTO ".$table." (link_id, title, url, description, visits, date_added)
                                 VALUES (NULL , '".strip_tags($_POST['title'])."', '".urldecode($_POST['url']) ."', '"
                                 .strip_tags($_POST['description'])."', '0', '".time()."')");
    if ($ld_repeated_link == 0){
     $sql      = $wpdb->prepare("SELECT count(url) FROM ".$table." WHERE url = '".urldecode($_POST['url']."'"));
